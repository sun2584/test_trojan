...
// BenchmarkClientServer tests the performance of trojan-go client and server
...
func BenchmarkClientServer(b *testing.B) {
	...
	serverPort := common.PickPort("tcp", "127.0.0.1")
	socksPort := common.PickPort("tcp", "127.0.0.1")
	clientDataTemplate := fmt.Sprintf(`
run-type: client
local-addr: 127.0.0.1
local-port: %d
remote-addr: 127.0.0.1
...
ssl:
    ...
    sni: localhost
`, socksPort, serverPort)
	serverDataTemplate := fmt.Sprintf(`
run-type: server
local-addr: 127.0.0.1
local-port: %d
remote-addr: 127.0.0.1
...
`, serverPort, util.HTTPPort)
	...
	var wg sync.WaitGroup // use WaitGroup to control goroutines synchronization

	for i := 0; i < n; i++ { // loop n times to simulate n clients
		wg.Add(1) // add one goroutine to the WaitGroup

		go func() { // use goroutine to run each client test concurrently
			defer wg.Done() // mark the goroutine as done when it finishes

			socksPort := common.PickPort("tcp", "127.0.0.1") // assign a random port for each client

			password := RandString(8) // generate a random password for each client

			fingerprint := RandFingerprint() // generate a random fingerprint for each client

			clientData := fmt.Sprintf(clientDataTemplate, socksPort, password, fingerprint) // fill in the placeholders with the values of each client

			serverData := fmt.Sprintf(serverDataTemplate, password) // fill in the placeholder with the value of each client

			b.Run(fmt.Sprintf("client%d", i), func(b *testing.B) { // use b.Run to run each subtest with a name and a function
				SingleThreadBenchmark(clientData, serverData, socksPort) // call SingleThreadBenchmark function to run the test for a single client

				b.ReportMetric(float64(b.N), "requests") // report the number of requests as a metric

				// b.ReportMetric(float64(b.Bytes)/float64(b.N), "bytes/op") // report the average bytes per request as a metric

				// b.ReportMetric(float64(b.NsPerOp()), "ns/op") // report the average nanoseconds per request as a metric

				// b.ReportMetric(float64(b.Bytes)/float64(b.NsPerOp()), "MB/s") // report the average megabytes per second as a metric
...
func RandFingerprint() string {
	var fingerprints = []string{"chrome", "firefox", "ios", "randomized"}
